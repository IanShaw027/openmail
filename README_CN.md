...
- 设备扫码同步：PC ↔ 手机保险库（服务端仅中转短时密文）  
- 2FA：TOTP/HOTP/Steam、扫码/粘贴/批量 URI、服务品牌图标、按服务筛选、拖拽排序、绑定邮箱  
- 控制台：批量导入、邮箱品牌标记、分组、备注、批量取件、验证码提取、发信、正文弹窗  
- 邮件列表：首次 20 封，加载更多（更早 10 封）/ 清空后重拉最近 20 封  
